...
class VendorPayouts
{
    ...
    protected function populateGetListConditions(array $params)
    {
        ...
        if (!empty($params['payout_id'])) {
            $condition .= $this->db->quote(' AND payout_id = ?i', $params['payout_id']);
        }

        if (!empty($params['order_id'])) {
            $condition .= $this->db->quote(' AND order_id IN (?n)', (array)$params['order_id']);
        }
        ...
        if (!empty($params['payout_type'])) {
            if (is_array($params['payout_type']) && in_array(VendorPayoutTypes::OTHER, $params['payout_type'])
                ...
                $condition .= $this->db->quote(
                    ' AND (payout_type NOT IN (?a) OR payout_type IN (?a))',
                    VendorPayoutTypes::getAll(),
                    (array)$params['payout_type']
                );
            } else {
                $condition .= $this->db->quote(' AND payout_type IN (?a)', (array)$params['payout_type']);
            }

        }

        if (!empty($params['approval_status'])) {
            $condition .= $this->db->quote(' AND approval_status IN (?a)', (array)$params['approval_status']);
        }
